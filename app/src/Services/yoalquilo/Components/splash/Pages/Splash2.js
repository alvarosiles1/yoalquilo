...
class Splash2 extends Component {
		...
		return <SForm
			...
			inputs={{
				cantHabitaciones: {
					...
					isRequired: true, keyboardType: "number", autoCapitalize: "none", type: "number", autoFocus: true, onKeyPress: (evt) => {
						if (evt.key === "Enter") {
	...
	render() {
		return (
			<SPage title={'Paso 2'}>
				<SView center col={"xs-12"} height height >
					<SView col={"xs-11 md-6 lg-4 xl-4"} center height>
						<SHr height={40} />

						<SView col={"xs-12"} height={100} center  >
							<SText color={'#1F2937'} fontSize={24} center >{'Con cuanto habitaciones, departametos contamos?'}</SText>
						</SView>

						<SView col={"xs-12"} height={18} />

						<SView col={"xs-12"} height={70} >
							{this.getForm()}
						</SView>

						<SView col={"xs-12"} height={32} />

						<SView col={"xs-12"} center flex height={190}  >
							<SView col={"xs-12"}   >
								<SIcon name={"Splash002"} />
							</SView>
						</SView>
						...
						<SView col={"xs-12"} height={80} row center    >
							...
							<SView col={"xs-2"} height center style={{ transform: [{ rotate: "180deg", }] }}
								onPress={() => { SNavigation.navigate("splash3") }}
							>
...
export default connect(initStates)(Splash2);